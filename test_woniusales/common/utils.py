...
def get_result(version):
    ...
    for i in res:
        summery_all[i[3]] += 1
        if i[0] in summery_module:
            summery_module[i[0]]['Total'] += 1
        else:
            summery_module[i[0]] = {}
            summery_module[i[0]]['Total'] = 1
            summery_module[i[0]].update({'Pass': 0, 'Fail': 0, 'Skip': 0})  # 字典的update方法，把另一个字典添加到这个字典中

        summery_module[i[0]][i[3]] += 1
    ...
    with open(file, 'r', encoding='utf8') as f:
        ...
        for i, j in enumerate(res):
            tmp = f'''<tr><td class="bottom">{i + 1}</td>
                <td class="case">{j[1]}</td>
                <td class="bottom">{j[0]}</td>
                <td class="case">{j[2]}</td>
                <td class="bottom">{j[3]}</td>
            </tr>'''
            detail += tmp
        s = s.replace('&detail', detail)
